...
# v0.2.0.0-rc2
# REMOVED ORDER_BY dict and list support
#
# selects.append(
...
selects.append(
    db.select(TABLE='users', WHERE={'group_id': 4})
)
...
logger.info(f"\nOne record, one value: {selects.pop()}")
...
logger.info(f"\nAll from users where_ group_id=1 and username=user_1, order_by username: {selects.pop()}")
logger.info(f"\nAll from users where_ group_id=1: {selects.pop()}")
logger.info(f"\nUsernames from users: {selects.pop()}")
logger.info(f"\nUsernames and group_id from users: {selects.pop()}")
logger.info(f"\nAll from users: {selects.pop()}")


####################################################
# SELECT data from DB
db.insertmany("users", [10, 1], [11, 2], [12, 2])
db.insertmany("users", [(20, 1), (21, 2), (23, 2)])
